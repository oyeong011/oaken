...
처음에는 Oaken 논문이 제기한 KV-cache quantization 문제의식에서 출발했습니다. 다만 제가 한 것은 논문 전체 재현이 아니라, Oaken-inspired 방식으로 consumer GPU에서 KV-cache memory pressure를 직접 관찰한 실험입니다. RTX 5060 8GB에서는 OPT-1.3B와 Qwen2.5-1.5B-Instruct를 대상으로 batch size와 sequence length를 바꾸며 dynamic, quantized, no_cache를 비교했습니다. Qwen 실험에서는 `position_valid=True`, `kv_formula_type=gqa_mqa`, dynamic `kv_actual_over_theory=1.0`을 확인한 뒤, `batch=8, seq_len=12288/16384`에서 dynamic OOM과 quantized rescue를 기록했습니다 (`results/rtx5060_qwen25_15b_*`). RTX 5080에서는 같은 Qwen cache-policy sweep이 아니라, OPT-125M부터 OPT-6.7B까지의 Oaken-style Wikitext accuracy path와 VRAM boundary artifact가 있습니다 (`results/oaken_consumer_gpu_summary.csv`). 배운 점은 KV-cache optimization은 단순 speedup이 아니라 memory-capacity trade-off로 보아야 하고, 더 큰 GPU도 boundary를 없애는 것이 아니라 더 큰 model size로 boundary를 이동시킨다는 것입니다.
...
현재 repo-backed Qwen cross-GPU cache-policy 비교는 아직 없습니다. RTX 5060은 dynamic/quantized/no_cache cache-policy sweep evidence이고, RTX 5080은 OPT model size를 키운 Oaken-style Wikitext accuracy artifact와 VRAM boundary evidence입니다. Repo 안의 RTX 5080 summary는 OPT-125M, OPT-350M, OPT-1.3B, OPT-2.7B Oaken Wikitext evaluation 완료와 OPT-6.7B boundary를 기록합니다 (`results/oaken_consumer_gpu_summary.csv`). 특히 OPT-6.7B는 original eval/profiling은 expandable allocator로 완료됐지만 Oaken eval은 약 15.8GB peak VRAM 근처에서 CUDA OOM이었습니다 (`results/rtx5080/opt-6.7b/summary.md`).
...
Qwen2.5-1.5B를 RTX 5080에서 같은 cache-policy 조건으로 실행해 5060에서 OOM이던 `8x12288`, `8x16384`가 dynamic으로 살아나는지 확인하는 것입니다. 현재 repo의 5080 근거는 Qwen sweep이 아니라 OPT Oaken-style boundary이므로, Qwen 5080 CSV를 추가해야 cross-GPU cache-policy 비교를 주장할 수 있습니다.
...
10. RTX 5080 evidence는 Qwen cache-policy sweep이 아니라 OPT Oaken-style Wikitext/VRAM boundary입니다.
...
| RTX 5080에서도 같은 cache-policy sweep이 검증됐습니다 | repo의 5080 파일은 OPT Oaken-style Wikitext/VRAM boundary 근거임 | 5080은 OPT model-size boundary 근거이고, 5060은 Qwen/OPT cache-policy sweep 근거입니다 |
